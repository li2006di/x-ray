...
get_latest_version() {
  ...
  local tmp_file
  tmp_file="$(mktemp)"
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$tmp_file" 'https://api.github.com/repos/man2018/x-ray/releases/latest'; then
    "rm" "$tmp_file"
  ...
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$tmp_file" 'https://api.github.com/repos/man2018/x-ray/releases'; then
    "rm" "$tmp_file"
    echo 'error: Failed to get release list, please check your network.'
    exit 1
  fi
  ...
  do
    ...
    grep -q "https://github.com/man2018/x-ray/releases/download/${releases_list[$i]}/Xray-linux-$MACHINE.zip" "$tmp_file" && break
  done
  "rm" "$tmp_file"
...
download_xray() {
  DOWNLOAD_LINK="https://github.com/man2018/x-ray/releases/download/$INSTALL_VERSION/Xray-linux-$MACHINE.zip"
...
install_geodata() {
  ...
  local download_link_geoip="https://github.com/man2018/x-ray/raw/main/geoip/geoip.dat"
  local download_link_geosite="https://github.com/v2fly/domain-list-community/releases/latest/download/dlc.dat"
  #local download_link_geosite="https://raw.githubusercontent.com/man2018/x-ray/main/geoip/dlc.dat"
